...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="/">
			 ...
			 -->
				...
				<xsl:if test="fn:exists(//dc:date[4])">
					<xsl:variable name="in" select="//dc:date[4]"/>
					<mtd3-br:dateIssued>
						<xsl:value-of select="concat(substring($in,7,4),'-',substring($in,4,2),'-',substring($in,1,2))"/>
					</mtd3-br:dateIssued>
				</xsl:if>
				...
				<xsl:variable name="programa" select="substring-before(substring-after(//dc:identifier[1],concat($departamento,',')),'.')"/>
				<xsl:variable name="programa1" select="substring-before(substring-after(//dc:identifier[1],concat($programa,'.')),'.')"/>
				
				
				<mtd3-br:publisher>
					<xsl:text>Universidade Federal do Par&#225;</xsl:text>
				</mtd3-br:publisher>
				...
				<xsl:if test="fn:exists(//dc:identifier[1])">
					<mtd3-br:publisherDepartament>
						<xsl:value-of select="normalize-space($departamento)"/>
					</mtd3-br:publisherDepartament>
				</xsl:if>
			   
			    <xsl:if test="fn:exists(//dc:identifier[1])">
					<mtd3-br:publisherProgram>
						<xsl:value-of select="normalize-space($programa1)"/>
					</mtd3-br:publisherProgram>
				</xsl:if>
	
				<mtd3-br:publisherInitials>
					<xsl:text>UFPA</xsl:text>
				</mtd3-br:publisherInitials>
