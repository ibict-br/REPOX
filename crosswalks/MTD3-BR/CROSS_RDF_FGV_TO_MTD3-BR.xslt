...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="/">	
	<xsl:if test="fn:exists(//dc:coverage)">
...
<!--
			<xsl:if test="fn:exists(//dc:description[5])">
				<mtd3-br:descriptionSponsorship>
					<xsl:value-of select="//dc:description[5]"/>
				</mtd3-br:descriptionSponsorship>
			</xsl:if>
			-->
			...
			<mtd3-br:publisher>
				<xsl:text>Funda&#231;&#227;o Getulio Vargas</xsl:text>
			</mtd3-br:publisher>
			...
			<xsl:if test="not(fn:exists(//dc:type))">
				<mtd3-br:type> 
					<xsl:call-template name="vmf:vmf3_inputtotype">
						<xsl:with-param name="input" select="translate(string(//dc:source[2]), $smallcase, $uppercase)"/>
					</xsl:call-template>
				</mtd3-br:type>
			</xsl:if>
			...
			<xsl:if test="not(fn:exists(//dc:language))">
				<mtd3-br:language>
					<xsl:text>por</xsl:text>
				</mtd3-br:language>
			</xsl:if>	
