...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template name="vmf:normaliza_type">
		...
		<xsl:choose>
			...
			<xsl:when test="((contains($inputType,'cap??tulo') or contains($inputType,'capitulo')
			...
			<xsl:when test="($inputType='livro' or $inputType='book' or contains($inputType,'livroscient??ficos(alice)') 
			or contains($inputType,'livro/book') or contains($inputType,'produ????obibliogr??fica:livros') or contains($inputType,'livro/folheto'))">
				<xsl:value-of select="'info:eu-repo/semantics/book'"/>
			</xsl:when>
			
			<xsl:when test="($inputType='publishedwork' or $inputType='peri??dico' or $inputType='revista' or $inputType='jornal' 
				or $inputType='fasc??culodeperi??dico' or contains($inputType,'textoemjornalourevista'))">
				<xsl:value-of select="'info:eu-repo/semantics/contributionToPeriodical'"/>
			</xsl:when>
			
			<xsl:when test="($inputType='tcc' or $inputType='bachelorThesis' or contains($inputType,'conclus??odeinicia????ocient??fica') 
			or contains($inputType,'conclus??odeoutranatureza') or contains($inputType,'gradua????o') or contains($inputType,'monografia') 
			or contains($inputType,'especializa????o'))">
			...
			<xsl:when test="((contains($inputType,'doutor') or contains($inputType,'doctor') or contains($inputType,'tese') 
			    or contains($inputType,'thesis') or contains($inputType,'p??s-doutorado')) and
				not(contains($inputType,'teses/disserta????es') or contains($inputType,'teseoudisserta????o') 
			...
			<xsl:when test="((contains($inputType,'mestre') or contains($inputType,'mestra') or contains($inputType,'master') 
			     or contains($inputType,'disserta')) and not(contains($inputType,'teses/disserta????es') or contains($inputType,'teseoudisserta????o')
				 or contains($inputType,'eletronicthesisordissertation')))">
			...
			<xsl:when test="(contains($inputType,'workingpapers,resenhas,entreoutros') or contains($inputType,'trabalhost??cnicos') 
			or contains($inputType,'textoparadiscuss??o'))">
				<xsl:value-of select="'info:eu-repo/semantics/workingPaper'"/>
			</xsl:when>
	
			<xsl:when test="($inputType='textodeapresenta????o/encerramento' or $inputType='lecture'  
			or $inputType='apresenta????o' or $inputType='apresentacao' or $inputType='relatodeexperiencia' or $inputType='presentation')">
			...
			$inputType='preprint' or contains($inputType,'artigosaceitosparapublica????o'))">
				<xsl:value-of select="'info:eu-repo/semantics/preprint'"/>
			</xsl:when>
			
			<xsl:when test="((contains($inputType,'relat??rio') or contains($inputType,'notat??cnica') or contains($inputType,'report')) and not(contains($inputType,'relat??rioadministrativo')))">
				<xsl:value-of select="'info:eu-repo/semantics/report'"/>
			</xsl:when>
			
			<xsl:when test="(contains($inputType,'novasescriturasemedia') or contains($inputType,'video') or contains($inputType,'v??deo')
			or $inputType='outro' or $inputType='outros' or $inputType='other' or $inputType='audio' or $inputType='text' or $inputType='texto'
			or $inputType='outraspublica????escient??ficas(alice)' or $inputType='produ????ot??cnica:outraprodu????ot??cnica' 
			or $inputType='produ????ot??cnica:outraprodu????ot??cnica' or $inputType='editorial' or $inputType='modelo3d' 
			...
			or contains($inputType,'tradu????o') or contains($inputType,'produ????ocultural') or contains($inputType,'produ????ocultural') 
