	...
	<xsl:template match="/">
		<mtd3-br:mtd3br xsi:schemaLocation="http://200.130.0.199/mtd3-br/mtd3-br.xsd" xmlns:mtd3-br="http://200.130.0.199/mtd3-br/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
			...
			<xsl:if test="fn:exists(//dc:date[3])">
				<xsl:variable name="data" select="normalize-space(//dc:date[3])"/>
				<mtd3-br:dateIssued>
				<xsl:choose>
			...
			<xsl:if test="fn:exists(//dc:identifier[1])">
				<xsl:variable name="departamento" select="substring-before(substring-after(//dc:identifier[1],'Par&#225;,'),',')"/>
			...
			<xsl:if test="fn:exists(//dc:source[1])">
				<mtd3-br:publisherProgram>
					<xsl:value-of select="normalize-space(//dc:source[1])"/>
				</mtd3-br:publisherProgram>
			</xsl:if>
