...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="/">
		<xsl:if test="not(contains(upper-case(//dim:field[@mdschema='dc'][@element='type']),'ARTI'))">
			...
			<mtd3-br:mtd3br xsi:schemaLocation="http://200.130.0.199/mtd3-br/mtd3-br.xsd" xmlns:mtd3-br="http://200.130.0.199/mtd3-br/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
				...
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='contributor'][@qualifier='advisorco'][position()=1]">
					<mtd3-br:contributorAdvisor-co1>
						<xsl:value-of select="."/>
					</mtd3-br:contributorAdvisor-co1>
				</xsl:for-each>
				
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='contributor'][@qualifier='advisorco'][position()=2]">
					<mtd3-br:contributorAdvisor-co2>
						<xsl:value-of select="."/>
					</mtd3-br:contributorAdvisor-co2>
				</xsl:for-each>
				
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='contributor'][@qualifier='member'][position()=1]">
					<mtd3-br:contributorReferees1>
						<xsl:value-of select="."/>
					</mtd3-br:contributorReferees1>
				</xsl:for-each>
				
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='contributor'][@qualifier='member'][position()=2]">
					<mtd3-br:contributorReferees2>
						<xsl:value-of select="."/>
					</mtd3-br:contributorReferees2>
				</xsl:for-each>
				
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='contributor'][@qualifier='member'][position()=3]">
					<mtd3-br:contributorReferees3>
						<xsl:value-of select="."/>
					</mtd3-br:contributorReferees3>
				</xsl:for-each>
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='contributor'][@qualifier='member'][position()=4]">
					<mtd3-br:contributorReferees4>
						<xsl:value-of select="."/>
					</mtd3-br:contributorReferees4>
				</xsl:for-each>
				
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='contributor'][@qualifier='member'][position()=5]">
				...
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='description'][@qualifier='abstracten']">
					<mtd3-br:descriptionAbstract>
				...
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='description'][@qualifier='abstractes']">
					<mtd3-br:descriptionAbstract>
				...
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='description'][@qualifier='abstractfr']">
					<mtd3-br:descriptionAbstract>
			...
			<!-- <xsl:for-each select="//dim:field[@mdschema='dc'][@element='degree'][@qualifier='grantor']"> -->
				<mtd3-br:publisher>
					<xsl:text>Fundação Osvaldo Cruz</xsl:text>
				</mtd3-br:publisher>
			<!--	</xsl:for-each> -->
				...
				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='degree'][@qualifier='department']">
					<mtd3-br:publisherDepartament>									
						<xsl:value-of select="."/>
					</mtd3-br:publisherDepartament>
				</xsl:for-each>
				
				<xsl:if test="not(fn:exists(//dim:field[@mdschema='dc'][@element='degree'][@qualifier='department']))">
					<xsl:for-each select="//dim:field[@mdschema='dc'][@element='degree'][@qualifier='grantor']">
						<mtd3-br:publisherDepartament>									
							<xsl:value-of select="."/>
						</mtd3-br:publisherDepartament>
				...
				<xsl:if test="not((fn:exists(//dim:field[@mdschema='dc'][@element='degree'][@qualifier='department']) or fn:exists(//dim:field[@mdschema='dc'][@element='degree'][@qualifier='grantor'])))">
					<mtd3-br:publisherDepartament>									
						<xsl:text>Funda&#231;&#227;o Oswaldo Cruz</xsl:text>
					</mtd3-br:publisherDepartament>
				</xsl:if>

				<xsl:for-each select="//dim:field[@mdschema='dc'][@element='degree'][@qualifier='program']">
