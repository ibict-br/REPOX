...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="/">
		...
		-->	
			...
			<mtd3-br:publisher>
				<xsl:text>Universidade Federal de Lavras</xsl:text>
			</mtd3-br:publisher>
			...
			<xsl:variable name="programaPosGraduacao" select="substring-before(substring-after(//dc:description[1],'ncias do '),',')"/>
		   
			<xsl:if test="fn:exists(//dc:description[1])">
				<mtd3-br:publisherProgram>
					<xsl:value-of select="$programaPosGraduacao"/>
				</mtd3-br:publisherProgram>
			</xsl:if>
			
			<mtd3-br:publisherInitials>
				<xsl:text>UFLA</xsl:text>
			</mtd3-br:publisherInitials>
			...
			<xsl:if test="not(fn:exists(//dc:type))">
				<mtd3-br:type> 
					<xsl:call-template name="vmf:vmf3_inputtotype">
						<xsl:with-param name="input" select="translate(string(substring(//dc:description[1],0,15)), $smallcase, $uppercase)"/>
					</xsl:call-template>
				</mtd3-br:type>
			</xsl:if>
