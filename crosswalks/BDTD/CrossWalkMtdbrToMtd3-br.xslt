...
exclude-result-prefixes="ns0 vmf xs fn ">
	...
	<xsl:template match="/">
		...
		-->	
			...
			<xsl:for-each select="$var1_mtdbr/ns0:Programa/ns0:Area">
				<mtd3-br:subjectCnpq>
					<xsl:sequence select="fn:string(.)"/>
				</mtd3-br:subjectCnpq>
			</xsl:for-each>
			<xsl:for-each select="$var1_mtdbr">
				<xsl:variable name="var27_resultof_filter" as="node()*">
					<xsl:for-each select="ns0:BibliotecaDigital/ns0:URLDocumento[fn:exists(@Formato)]">
						<xsl:variable name="var25_resultof_vmf__inputtoresult" as="xs:string?">
							<xsl:call-template name="vmf:vmf8_inputtoresult">
								<xsl:with-param name="input" select="fn:string(@Formato)" as="xs:string"/>
							</xsl:call-template>
						</xsl:variable>
