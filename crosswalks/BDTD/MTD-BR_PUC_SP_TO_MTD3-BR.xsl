...
exclude-result-prefixes="ns0 vmf xs fn">
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="$var1_mtdbr/ns0:BibliotecaDigital">
			<mtd3-br:identifierUri>
				<xsl:sequence select="xs:string(xs:anyURI(fn:string(ns0:URLDocumento)))"/>
			</mtd3-br:identifierUri>
		</xsl:for-each>
		...
		<xsl:if test="not(contains($var1_mtdbr/ns0:AgenciaFomento/ns0:Nome,'nenhum'))">
		...
		<xsl:for-each select="$var1_mtdbr">
			<mtd3-br:publisher>
				<xsl:sequence select="fn:string(ns0:Programa/ns0:Instituicao/ns0:Nome)"/>
			</mtd3-br:publisher>
		</xsl:for-each>		
		<xsl:for-each select="$var1_mtdbr/ns0:Programa/ns0:Instituicao/ns0:Pais">
		...
		<xsl:for-each select="$var1_mtdbr">
			<mtd3-br:publisherDepartament>
				<xsl:sequence select="fn:string(ns0:Programa/ns0:Area)"/>
			</mtd3-br:publisherDepartament>
		</xsl:for-each>
		<xsl:for-each select="$var1_mtdbr/ns0:Programa">
			<mtd3-br:publisherProgram>
				<xsl:sequence select="fn:string(ns0:Area)"/>
			</mtd3-br:publisherProgram>
		</xsl:for-each>
		<xsl:for-each select="$var1_mtdbr/ns0:Programa/ns0:Instituicao/ns0:Sigla">
		...
		<xsl:for-each select="$var1_mtdbr">
			<xsl:variable name="var27_resultof_filter" as="node()*">
				<xsl:for-each select="ns0:BibliotecaDigital[fn:exists(ns0:URLDocumento/@Formato)]">
					<xsl:variable name="var25_resultof_vmf__inputtoresult" as="xs:string?">
						<xsl:call-template name="vmf:vmf8_inputtoresult">
							<xsl:with-param name="input" select="fn:string(ns0:URLDocumento/@Formato)" as="xs:string"/>
						</xsl:call-template>
					</xsl:variable>
