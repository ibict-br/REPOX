...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template name="vmf:vmf3_inputtotype">
		...
		<xsl:if test="(contains($input,'MESTRE')) or (contains($input,'MESTRA')) or (contains($input,'MASTER')) or (contains($input,'DISSERTA')) or ($input='DISSERTATION') or ($input='ME') or ($input='METRE')">
	...
	-->
	<xsl:template match="/">
		<xsl:if test="not((contains(upper-case(marc:datafield[@tag=696]/marc:subfield[@code='a']),'DOCÊNCIA'))
					  or (contains(upper-case(marc:datafield[@tag=696]/marc:subfield[@code='a']),'LD'))
					  or (contains(upper-case(marc:datafield[@tag=696]/marc:subfield[@code='a']),'LIVRE'))						  
					  or (contains(upper-case(marc:datafield[@tag=696]/marc:subfield[@code='a']),'DOCENTE')))">
	...
	<xsl:template match="marc:record">
		<xsl:if test="not((contains(upper-case(marc:datafield[@tag=696]/marc:subfield[@code='a']),'DOCÊNCIA'))
			...
			<mtd3-br:publisher>
				<xsl:text>Universidade Estadual Paulista "J&#250;lio de Mesquita Filho"</xsl:text>
			</mtd3-br:publisher>
