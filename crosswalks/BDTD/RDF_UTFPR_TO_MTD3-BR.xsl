...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="/">
		<mtd3-br:mtd3br xsi:schemaLocation="http://200.130.0.199/mtd3-br/mtd3-br.xsd" xmlns:mtd3-br="http://200.130.0.199/mtd3-br/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
			...
			<xsl:if test="(fn:count(//dc:degree)=5)">
				<xsl:variable name="data" select="normalize-space(//dc:degree[4])"/>
				<mtd3-br:dateIssued>
					<xsl:choose>
		...
		<xsl:if test="(fn:count(//dc:degree)&lt; 5)">
			<!--
			<xsl:variable name="dataNormalize" select="normalize-space(//dc:identifier[1])"/>
			<xsl:variable name="positionData" select="string-length($dataNormalize)-4"/>
			<xsl:variable name="data" select="substring($dataNormalize,$positionData,4)"/>
			-->
			<xsl:variable name="data" select="substring(normalize-space(//dc:identifier[1]),string-length(normalize-space(//dc:identifier[1]))-4,4)"/>
				<mtd3-br:dateIssued>
			...
			<mtd3-br:publisher>
				<xsl:text>Universidade Tecnol&#243;gica Federal do Paran&#225;</xsl:text>
			</mtd3-br:publisher>
			...
			<xsl:if test="fn:exists(//dc:degree[1])">
				<mtd3-br:publisherDepartament>
					<xsl:value-of select="normalize-space(//dc:degree[1])"/>
				</mtd3-br:publisherDepartament>
			</xsl:if>
		   
			<xsl:if test="fn:exists(//dc:degree[2])">
				<mtd3-br:publisherProgram>
					<xsl:value-of select="normalize-space(//dc:degree[2])"/>
				</mtd3-br:publisherProgram>
			</xsl:if>

			<mtd3-br:publisherInitials>
				<xsl:text>UTFPR</xsl:text>
			</mtd3-br:publisherInitials>
