...
<xsl:stylesheet version="2.0" 
    ...
    exclude-result-prefixes="oai_dc dc rdf vmf fn">	
	...
	<xsl:template match="/">
		...
		<mtd3-br:mtd3br xsi:schemaLocation="http://200.130.0.199/mtd3-br/mtd3-br.xsd" xmlns:mtd3-br="http://200.130.0.199/mtd3-br/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
		 ...
		 <!--
			...
			<xsl:if test="fn:exists(//dc:contributor[7])">
				<mtd3-br:contributorReferees5>
					<xsl:value-of select="//dc:contributor[7]"/>
				</mtd3-br:contributorReferees5>
			</xsl:if>		
			...
			<xsl:if test="not(fn:exists(//dc:date[2]))">
				<mtd3-br:dateIssued>
					<xsl:value-of select="//dc:date[1]"/>
				</mtd3-br:dateIssued>
			</xsl:if>
			...
			<xsl:if test="fn:exists(//dc:identifier[1]) and fn:exists(//dc:identifier[2])">
			...
			-->
			<!--
			<xsl:variable name="instituicao" select="substring-before(substring-after(//dc:identifier[1],' - '),',')"/>
			<xsl:variable name="departamento" select="substring-before(substring-after(//dc:identifier[1],concat($instituicao,',')),',')"/>
			<xsl:variable name="programaMestrado" select="substring-before(substring-after(//dc:identifier[1],'Mestrado em'),')')"/>
			<xsl:variable name="programaDoutorado" select="substring-before(substring-after(//dc:identifier[1],'Doutorado em'),')')"/>
			
			-->
			
			<mtd3-br:publisher>
				<xsl:text>Universidade Municipal de S&#227;o Caetano do Sul</xsl:text>
			</mtd3-br:publisher>
			...
			<xsl:if test="fn:exists(//dc:source[2])">
				<mtd3-br:publisherDepartament>
					<xsl:value-of select="//dc:source[2]"/>
				</mtd3-br:publisherDepartament>
			</xsl:if>
		   
			<xsl:if test="fn:exists(//dc:source[1])">
				<mtd3-br:publisherProgram>
					<xsl:value-of select="//dc:source[1]"/>
				</mtd3-br:publisherProgram>
			</xsl:if>
			
			
			<mtd3-br:publisherInitials>
				<xsl:text>USCS</xsl:text>
			</mtd3-br:publisherInitials>

	<!--
			<xsl:if test="fn:exists(//dc:publisher[1])">
				<mtd3-br:publisherInitials>
	...
	-->
			...
			<xsl:if test="contains(//dc:description[1],'Dissertação (Doutorado')">
				<mtd3-br:type>
					<xsl:text>doctoralThesis</xsl:text>
				</mtd3-br:type>
			</xsl:if>
		
			<xsl:if test="fn:exists(//dc:description[1]) and not(contains(//dc:description[1],'Dissertação (Doutorado'))">
				<mtd3-br:type>
			...
			<xsl:if test="fn:exists(//dc:rights[1])">
				<mtd3-br:rights>
					<xsl:value-of select="//dc:rights[1]"/>
				</mtd3-br:rights>
			</xsl:if>


			<xsl:if test="fn:exists(//dc:rights[2])">
				<mtd3-br:rightsUri>
					<xsl:value-of select="//dc:rights[2]"/>
				</mtd3-br:rightsUri>
			</xsl:if>				
